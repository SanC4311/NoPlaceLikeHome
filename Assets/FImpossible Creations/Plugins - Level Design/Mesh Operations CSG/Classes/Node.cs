...
namespace Parabox.CSG
{
    sealed class Node
    {
        ...
        public List<Polygon> AllPolygons(bool flipNormals = false)
        {
            ...
            if (list != null)
            {
                ...
                if (flipNormals) FlipNormals(list);
            }
        ...
        void FlipNormals(List<Polygon> polygons)
        {
            for (int i = 0; i < polygons.Count; i++)
            {
                ...
                {
                    var vert = polygons[i].vertices[v];
                    vert.normal = -vert.normal;
                    polygons[i].vertices[v] = vert;
                }
            }
        }
        ...
        // solid `csg`. Neither this solid nor the solid `csg` are modified.
        public static Node Subtract(Node a1, Node b1, bool flipB = false)
        {
            Node a = a1.Clone();
            Node b = b1.Clone();
