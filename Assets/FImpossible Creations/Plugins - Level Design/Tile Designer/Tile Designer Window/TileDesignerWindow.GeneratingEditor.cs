    ...
    public partial class TileDesignerWindow
    {
        ...
        private void GUIEditor()
        {
            ...
            if (drawOptions)
            {
                ...
                if (GUILayout.Button(FGUI_Resources.Tex_DownFold, FGUI_Resources.ButtonStyle, GUILayout.Width(22), GUILayout.Height(17)))
                {
                    TileMeshSelectorGenericMenu();
                }
                ...
                if (EditedTileSetup.Material != null)
                {
                    if (EditedTileSetup.OverrideTag == "Untagged") GUI.color = new Color(1f, 1f, 1f, 0.6f);
                    EditorGUIUtility.labelWidth = 80;
                    EditedTileSetup.OverrideTag = EditorGUILayout.TagField("Override Tag:", EditedTileSetup.OverrideTag, GUILayout.Width(158));
        ...
        void TileMeshSelectorGenericMenu()
        {
            if (EditedDesign.TileMeshes.Count == 0) return;

            GenericMenu menu = new GenericMenu();

            for (int i = 0; i < EditedDesign.TileMeshes.Count; i++)
            {
                int idx = i;
                menu.AddItem(new GUIContent("[" + idx + "] " + EditedDesign.TileMeshes[i].Name), _selectedTileMesh == idx, () => { _selectedTileMesh = idx; EditedTileSetup = EditedDesign.TileMeshes[idx]; });
            }
