...
namespace FIMSpace.Generating.Checker
{
    public partial class CheckerField3D
    {
        ...
        Vector3 lastPosition = new Vector3(100000, -10000, 300303);
        Quaternion lastRotation = Quaternion.identity;
        Matrix4x4 _matrixOnDemand;
        Matrix4x4 _matrixOnDemand_NoScale;
        Matrix4x4 _invMatrixOnDemand;
        Matrix4x4 _invMatrixOnDemand_NoScale;

        void RefreshMatrix()
        {
            lastPosition = RootPosition;
            lastRotation = RootRotation;

            _matrixOnDemand = Matrix4x4.TRS(RootPosition, RootRotation, RootScale);
            _matrixOnDemand_NoScale = Matrix4x4.TRS(RootPosition, RootRotation, Vector3.one);
            _invMatrixOnDemand = _matrixOnDemand.inverse;
            _invMatrixOnDemand_NoScale = _invMatrixOnDemand_NoScale.inverse;
        }

        void MatrixReset()
        {
            lastPosition = Vector3.positiveInfinity;
            lastRotation = Quaternion.Euler(123.432f, 342.123f, 74.472f);
        }

        void MatrixChangeCheck()
        {
            if (RootPosition != lastPosition || lastRotation != RootRotation)
            {
                RefreshMatrix();
            }
        }
        ...
        public Matrix4x4 Matrix_NoScale
        {
            ...
            {
                MatrixChangeCheck();
                return _matrixOnDemand_NoScale;
                //return Matrix4x4.TRS(RootPosition, RootRotation, Vector3.one);
            }
        }

        public Matrix4x4 MatrixInverse_NoScale
        {
            get
            {
                MatrixChangeCheck();
                return _invMatrixOnDemand_NoScale;
            }
        }
        ...
        public List<FieldCell> BoundsToCells(Bounds localBounds, bool getRounded = true, bool generateIfOut = false, bool newListInstance = true/*, bool worldMatrix = true*/)
        {
            ...
            for (int x = lX; x <= rX; x++)
            {
                for (int y = dY; y <= uY; y++)
                {
                    for (int z = dZ; z <= uZ; z++)
                    {
                        FieldCell c = GetCell(new Vector3Int(x, y, z), generateIfOut);
        ...
        /// <summary> Bounds with size od 1 = 1 cell, converting it's volume into cells (rounded) in grid's local space </summary>
        public void JoinWithLocalBoundsToCells(Bounds localBounds, Vector3Int centerOffset)
        {
            int lX, rX, uZ, dZ, uY, dY;
            ...
            for (int x = lX; x <= rX; x++)
            {
                for (int y = dY; y <= uY; y++)
                {
                    for (int z = dZ; z <= uZ; z++)
                    {
                        Grid.AddCell(x - centerOffset.x, y - centerOffset.y, z - centerOffset.z);
                    }
                }
            }
        }
        ...
        /// <summary> Bounds with size od 1 = 1 cell, converting it's volume into cells (rounded) in grid's local space </summary>
        public void JoinWithLocalBoundsToCells(Bounds localBounds)
        {
            JoinWithLocalBoundsToCells(localBounds, Vector3Int.zero);
        }
        ...
        public bool IsCollidingWith(CheckerField3D other, bool checkRounded = false, float boundsMultiplier = 1f)
        {
            if (!UseBounds)
            {
                Bounds mInt = GetCollisionBounds(other, boundsMultiplier);
            ...
            else
            {
                for (int i = 0; i < Bounding.Count; i++)
                {
                    Bounds iWorld = Bounding[i];
                    iWorld.center += RootPosition;
                    iWorld.size *= 0.999f * boundsMultiplier;

                    for (int o = 0; o < other.Bounding.Count; o++)
                    {
                        Bounds oWorld = other.Bounding[o];
                        oWorld.center += other.RootPosition;
                        oWorld.size *= 0.999f * boundsMultiplier;
        ...
        public Bounds GetCollisionBounds(CheckerField3D other, float boundsMultiplier = 1f)
        {
            Bounds oB = other.GetFullBoundsWorldSpace();
            oB.size *= 0.999f * boundsMultiplier;
            Bounds mB = GetFullBoundsWorldSpace();
            if (boundsMultiplier != 1f) mB.size *= boundsMultiplier;
        ...
        {
            if (AllCells.Count == 0) return new Bounds(Vector3.zero, Vector3.zero);
