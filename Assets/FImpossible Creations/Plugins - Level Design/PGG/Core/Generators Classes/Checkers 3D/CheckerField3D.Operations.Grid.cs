...
namespace FIMSpace.Generating.Checker
{
    public partial class CheckerField3D
    {
        ...
        {
            if (invertMatrix == null) invertMatrix = MatrixInverse;
        ...
        public FieldCell GetCellInWorldPos(Vector3 worldPos, bool checkRounded, Matrix4x4? invertMatrix = null)
        {
            ...
            if (invertMatrix != null)
                ...
                locPos = MatrixInverse.MultiplyPoint3x4(worldPos);
        ...
        {
            return GetWorldPos(gridLocalPos, Matrix);
        }
        ...
        public Vector3 GetWorldPos(Vector3Int localPos, Matrix4x4 mx)
        {
            return mx.MultiplyPoint3x4(localPos);
        }
        ...
        /// <summary> Transforming other checker local position (world representation) to this checker local space </summary>
        public Vector3Int GetLocalPositionOfOther(Vector3Int otherLocal, CheckerField3D other)
        {
            Vector3 wPos = other.LocalToWorld(otherLocal);
            return WorldToLocal(wPos).V3toV3Int();
        }
