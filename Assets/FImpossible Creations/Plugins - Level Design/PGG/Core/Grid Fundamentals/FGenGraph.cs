...
namespace FIMSpace.Generating
{
    public partial class FGenGraph<T1, T2> where T1 : FGenCell, new() where T2 : FGenPoint, new()
    {
        ...
        public FieldCell GetNearestFrom(FieldCell startCell, FieldCell[] cells)
        {
        ...
        public bool CellIsDiagonalOut(FieldCell cell)
        {
        ...
        /// <summary> If cell is diagonal end edge, like ┌ </summary>
        public bool CellIsSurrounded(FieldCell cell, bool includeDiagonal = false)
        {
            Vector3Int myPos = cell.Pos;
        ...
        public bool CellIsSurroundedOnlyDiag(Vector3Int pos)
        {
        ...
        private void CheckForMinMax(T1 cell, bool isRemoving = false)
        {
            if (FGenerators.CheckIfExist_NOTNULL(cell))
            {
                ...
                {
                    if (MaxX == null || cell.Pos.x == MaxX.Pos.x)
                        /*if (cell == MaxX)*/ // Search for current max
                        MaxX = AnalyzeGridForMaxX();

                    if (MinX == null || cell.Pos.x == MinX.Pos.x)
                    ...
                    if (MinZ == null || cell.Pos.z == MinZ.Pos.z) /*if (cell == MinZ)*/ MinZ = AnalyzeGridForMinZ();
                }
            }
        }
        ...
        public void RestoreGrid(List<T1> gridCellsSave)
        {
        ...
        public FGenGraph<T1, T2> GetCorrespondingSubGraph(int scale)
        {
            if (SubGraphs != null)
        ...
        public T1 GetCorrespondingSubGraphCell(FieldCell cell, FGenGraph<T1, T2> rightGraph)
        {
        ...
        public Vector3 GetWorldCenter(Vector3 cellSize, bool withOffset = false)
        {
        ...
        public Vector3 GetCenterOffset(Vector3 cellSize)
        {
            cellSize *= ReferenceScale;
            float x = 0f;
            float y = 0f;
