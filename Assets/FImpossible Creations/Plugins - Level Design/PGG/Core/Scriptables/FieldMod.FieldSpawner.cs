...
namespace FIMSpace.Generating
{
    ...
    public partial class FieldSpawner
    {
        ...
        public void SetTemporaryPrefabToSpawn(GameObject p)
        {
        ...
        public void RemoveRule(SpawnRuleBase rule)
        {
        ...
        public SpawnData RunSpawnerRulesFor(SpawnData spawnData, FieldModification mod, FieldSetup preset, FieldCell cell, FGenGraph<FieldCell, FGenPoint> grid, Vector3? desiredDirection, FieldModification childMod, bool dontUseGlobalRules = false, SpawnRuleBase skipRule = null, bool newListInstance = false)
        {
            ...
            for (int i = 0; i < Rules.Count; i++)
            {
                ...
                //if (t != null) if (t.Type == SpawnRuleBase.EProcedureType.Rule || t.Type == SpawnRuleBase.EProcedureType.Coded) if (Rules[i].CellAllow == false) { _RulesToCheck.Clear(); break; }
        ...
        public bool IsExecutionReversed()
        {
        ...
        [NonSerialized] public List<Collider> spawnCollidersList = new List<Collider>();
        [NonSerialized] public List<MeshFilter> spawnMeshesList = new List<MeshFilter>();
        [NonSerialized] public Bounds spawnCollisionFullBounds;
        /// <summary> 0 is reconstructed using base spawner, 1 is reconstructed using overrided prefab </summary>
        private int collisionReconstructed = -1;
        ...
        public void PreGenerating()
        {
            ...
            if (spawnCollidersList == null) spawnCollidersList = new List<Collider>();
            spawnCollidersList.Clear();
            if (spawnMeshesList == null) spawnMeshesList = new List<MeshFilter>();
            spawnMeshesList.Clear();
            spawnCollisionFullBounds = new Bounds();
            collisionReconstructed = -1;
        ...
        public void CheckCollisionReconstruction(GameObject spawnerPrefab)
        {
            if (TemporaryPrefabOverride != null) if (collisionReconstructed == 1) return;
            if (collisionReconstructed == 0) return;

            Vector3 prePos = Vector3.zero;

            if (TemporaryPrefabOverride)
            {
                prePos = TemporaryPrefabOverride.transform.position;
                TemporaryPrefabOverride.transform.position = Vector3.zero;

                spawnMeshesList = GatherComponentsFrom<MeshFilter>(TemporaryPrefabOverride);
                spawnCollidersList = GatherComponentsFrom<Collider>(TemporaryPrefabOverride);
                collisionReconstructed = 1;
            }
            else
            {
                prePos = spawnerPrefab.transform.position;
                spawnerPrefab.transform.position = Vector3.zero;

                spawnMeshesList = GatherComponentsFrom<MeshFilter>(spawnerPrefab);
                spawnCollidersList = GatherComponentsFrom<Collider>(spawnerPrefab);
                collisionReconstructed = 0;
            }

            if (spawnCollidersList.Count > 0)
            {
                Physics.SyncTransforms();
                spawnCollisionFullBounds = new Bounds(spawnCollidersList[0].bounds.center, Vector2.zero);

                for (int i = 0; i < spawnCollidersList.Count; i++)
                {
                    spawnCollisionFullBounds.Encapsulate(spawnCollidersList[i].bounds);
                }
            }
            else
            {
                if (spawnMeshesList.Count > 0)
                {
                    if (spawnMeshesList[0].sharedMesh == null)
                        spawnCollisionFullBounds = new Bounds();
                    else
                        spawnCollisionFullBounds = new Bounds(spawnMeshesList[0].transform.TransformPoint(spawnMeshesList[0].sharedMesh.bounds.center), Vector2.zero);

                    for (int i = 0; i < spawnMeshesList.Count; i++)
                    {
                        if (spawnMeshesList[i].sharedMesh == null) continue;
                        Bounds b = FEngineering.TransformBounding(spawnMeshesList[i].sharedMesh.bounds, spawnMeshesList[i].transform.localToWorldMatrix);
                        spawnCollisionFullBounds.Encapsulate(b);
                    }
                }
            }

            if (TemporaryPrefabOverride) TemporaryPrefabOverride.transform.position = prePos;
            else if (spawnerPrefab) spawnerPrefab.transform.position = prePos;
        }

        public static List<T> GatherComponentsFrom<T>(GameObject obj) where T : Component
        {
            List<T> comps = new List<T>();
            foreach (var t in obj.GetComponentsInChildren<Transform>())
            {
                T cmp = t.GetComponent<T>();
                if (cmp) comps.Add(cmp);
            }
