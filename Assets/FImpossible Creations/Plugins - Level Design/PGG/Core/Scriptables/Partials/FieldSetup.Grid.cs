...
namespace FIMSpace.Generating
{
    public partial class FieldSetup : ScriptableObject
    {
        ...
        public void RunPreInstructionsOnGraph(FGenGraph<FieldCell, FGenPoint> grid, List<SpawnInstruction> guides)
        {
            if (guides != null)
            {
                IGeneration.RefreshModGraphParameters(guides);
        ...
        public FieldSetup GenerateRuntimeInstance()
        {
        ...
        public void RunCommandPostActionWithInstruction(FGenGraph<FieldCell, FGenPoint> grid, FieldModification modificator, SpawnInstruction guide)
        {
            ...
            for (int s = 0; s < modificator.Spawners.Count; s++) // Checking all mod's spawners
            {
                var spawner = modificator.Spawners[s];
                ...
                for (int i = 0; i < spawner.Rules.Count; i++)
                {
                    ...
                    spawner.Rules[i].ResetRule(grid, this);
                }

                for (int r = 0; r < spawner.Rules.Count; r++)
                {
                    var rule = spawner.Rules[r];
