...
namespace FIMSpace.Generating
{
    public abstract partial class SpawnRuleBase
    {
        ...
        public Vector3 GetCenteredSpawnPosition(SpawnData spawn, ESR_Origin targetOrigin)
        {
            Vector3 spawnPos = spawn.GetPosWithFullOffset(true);

            if (targetOrigin == ESR_Origin.SpawnPosition)
            {
                return spawnPos;
            }
            else if (targetOrigin == ESR_Origin.BoundsCenter)
            {
                ...
                {
                    return spawn.GetPosWithFullOffset(true) + spawn.GetRotationOffset() * Vector3.Scale(spawn.Prefab.transform.localScale, spawn.GetRotationOffset() * spawn.PreviewMesh.bounds.center);
                }
            }
            else // Renderer Center
            {
                Renderer otherSpawnRend = null;
                if (spawn.Prefab) otherSpawnRend = spawn.Prefab.GetComponentInChildren<Renderer>();
                ...
                {
                    return spawn.GetPosWithFullOffset(true) + spawn.GetRotationOffset() * Vector3.Scale(spawn.Prefab.transform.localScale, spawn.Prefab.transform.InverseTransformPoint(otherSpawnRend.bounds.center));
                }
            }
        }
        ...
        public static Bounds GetSpawnBounds(SpawnData spawn, bool applySpawnOffsets = true)
        {
            Bounds b = spawn.Spawner.spawnCollisionFullBounds;

            if (applySpawnOffsets)
            {
                b = FEngineering.TransformBounding(b, Matrix4x4.TRS(spawn.GetFullOffset(true), Quaternion.Euler(spawn.GetFullRotationOffset()), spawn.LocalScaleMul));
            }

            return b;
        }

        public static Vector3 GetSpawnCenterPositionCellSpace(SpawnData spawn, ESR_Origin DistanceSource)
        {
            Vector3 position = spawn.GetPosWithFullOffset();
            ...
            if (DistanceSource == ESR_Origin.BoundsCenter)
            {
                if (spawn.Prefab != null)
                {
                    if (spawn.PreviewMesh)
                        position = position + spawn.GetRotationOffset() * Vector3.Scale(spawn.Prefab.transform.localScale, spawn.PreviewMesh.bounds.center);
                }
            }
            else if (DistanceSource == ESR_Origin.RendererCenter)
            {
                if (spawn.Prefab != null)
                {
                    ...
                    if (thisRend)
                        position = position + spawn.GetRotationOffset() * Vector3.Scale(spawn.Prefab.transform.localScale, spawn.Prefab.transform.InverseTransformPoint(thisRend.bounds.center));
                    else
                        position = GetSpawnCenterPositionCellSpace(spawn, ESR_Origin.BoundsCenter);
                }
            }
            else if (DistanceSource == ESR_Origin.SpawnPosition)
            {
                return position;
            }
