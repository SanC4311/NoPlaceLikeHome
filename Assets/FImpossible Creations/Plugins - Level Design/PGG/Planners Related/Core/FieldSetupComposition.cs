...
namespace FIMSpace.Generating
{
    ...
    public partial class FieldSetupComposition
    {
        ...
        public List<InjectionSetup> Injections = new List<InjectionSetup>();
        ...
        /// <param name="selectedPack"> Can be -1 to not check mod pack </param>
        public bool ValidateComposition(int selectedPack)
        {
            if (Injections == null) Injections = new List<InjectionSetup>();
            ...
            for (int i = 0; i < FieldPackagesOverrides.Count; i++)
            {
                ...
                if (packOverr.PackVariablesOverrides == null) continue;
                if (Setup.ModificatorPacks[i] == null) continue;
                if (Setup.ModificatorPacks[i].Variables == null) continue;
        ...
        public void Clear()
        {
            ...
            Injections = new List<InjectionSetup>();
