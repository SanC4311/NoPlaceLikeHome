...
namespace FIMSpace.Generating.Planning.PlannerNodes.Field.Special
{
    ...
    public class PR_TightPlacement : PlannerRuleBase
    {
        ...
        public override void Execute(PlanGenerationPrint print, PlannerResult newResult)
        {
            ...
            _snapTo = GetPlannersFromPort(AlignTo, true, false);
                ...
                for (int i = 0; i < edgeCells.Count; i++)
                {
                    ...
                    for (int d = 0; d < dirs.Length; d++)
                    {
                        ...
                        for (int r = 0; r < rots; r++)
                        {
                            ...
                            if (skipIteration)
                            {
                                //FDebug.DrawBounds3D(checker.GetFullBoundsWorldSpace(), Color.red);
                                skipIteration = false;
                            }
                            else
                            {
                                ...
                                if (predictedPrintBounds.size.x != float.MaxValue)
                                {
                                    ...
                                    if (aligns >= mostAligns)
                                    {
                                        ...
                                        if (newBndsSize <= rootBndsSize)
                                        {
                                            ...
                                            found = GetContactCells(checker, alignToChecker, dirs[d], PushOutDistance, ref myCell, ref otherCell);
        ...
        protected virtual void OnFoundPlacementAndApply()
        {

        }


        public static bool GetContactCells(CheckerField3D my, CheckerField3D other, Vector3Int dir, float pushOutDistance, ref FieldCell myCell, ref FieldCell otherCell)
        {
            ...
            {
                UnityEngine.Debug.LogWarning("Not found alignment cell for bounded tight position (probably some cell scale calculation error), searching for another placement.");
        ...
        Bounds GetBoundsPlacement(CheckerField3D checker, CheckerField3D inline, FieldCell cell, CheckerField3D alignToChecker, List<CheckerField3D> fullMask, Vector3Int dir, bool breakOnAnyCollWithMask)
        {
            ...
            if (checker.RootScale.x % 1 != 0) // Fractions requires different rounding
