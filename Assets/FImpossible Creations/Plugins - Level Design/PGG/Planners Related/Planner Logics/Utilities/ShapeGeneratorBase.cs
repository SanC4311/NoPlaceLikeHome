...
namespace FIMSpace.Generating.Planning.GeneratingLogics
{
    ...
    public abstract partial class ShapeGeneratorBase : ScriptableObject
    {
        ...
        //protected FGenerators.DefinedRandom PGGRandom { get; private set; }
        ...
        protected SpawnInstructionGuide AddCellIntruction(FieldPlanner planner, FieldCell cell, int instructionID)
        {
            SpawnInstructionGuide instr = new SpawnInstructionGuide();
            instr.HelperCellRef = cell;
            instr.pos = instr.HelperCellRef.Pos;
            instr.Id = instructionID;
            planner.LatestResult.CellsInstructions.Add(instr);
            return instr;
        }
