...
namespace FIMSpace.Generating
{
    ...
    public partial class BuildPlannerExecutor : MonoBehaviour
    {
        ...
        public GUIContent[] GetSetupsNameList(bool forceRefresh = false)
        {
            if (BuildPlannerPreset == null) return null;
            if (_SetupPreset != BuildPlannerPreset) forceRefresh = true;
    ...
    public class BuildPlannerExecutorEditor : UnityEditor.Editor
    {
                ...
                if (allowDisplaySetup)
                {
                    ...
                    if (Get.BuildPlannerPreset.BasePlanners.Count > 0)
                    {
                        ...
                        if (!filled)
                        {
                            ...
                            FieldSetupComposition anyFilled = null;
                            for (int i = 0; i < Get.PlannerPrepare.FieldSetupCompositions.Count; i++)
                            {
                                if (Get.PlannerPrepare.FieldSetupCompositions[i].IsSettedUp) { anyFilled = Get.PlannerPrepare.FieldSetupCompositions[i]; break; }
                            }
                            ...
                            for (int i = 0; i < Get.PlannerPrepare.FieldSetupCompositions.Count; i++)
                            {
                                ...
                                if (Get.BuildPlannerPreset.BasePlanners[i] != null)
                                {
                                    ...
                                    DrawGenTypeField(compos, Get.BuildPlannerPreset.BasePlanners[i].name, anyFilled);
        ...
        private GUIContent _guic_get = null;
        public bool DrawGenTypeField(FieldSetupComposition compos, string title, FieldSetupComposition any = null)
        {
            _drawGenTypeChanged = false;
            ...
            if (compos.IsSettedUp == false)
                if (any != null && any != compos)
                {
                    if (_guic_get == null || _guic_get.image == null) { _guic_get = new GUIContent(FGUI_Resources.Tex_UpFold, "Get setup of the already assigned reference."); }

                    if (GUILayout.Button(_guic_get, FGUI_Resources.ButtonStyle, GUILayout.Height(19), GUILayout.Width(21)))
                    {
                        compos.Setup = any.Setup;
                        compos.JustMod = any.JustMod;
                        compos.JustModPack = any.JustModPack;
                        compos.GenType = any.GenType;
                        _drawGenTypeChanged = true;
                    }
                }
