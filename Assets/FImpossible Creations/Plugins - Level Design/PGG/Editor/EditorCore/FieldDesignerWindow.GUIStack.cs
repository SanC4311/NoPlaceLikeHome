...
namespace FIMSpace.Generating
{
    public partial class FieldDesignWindow
    {
            ...
            if (advancedMode)
            {
                ...
                if (drawPreviewSetts)
                {
                    ...
                    if (drawTestGenSetts)
                    {
                        ...
                        if (advancedMode)
                        {
                            gridMode = (EDesignerGridMode)EditorGUILayout.EnumPopup("Preview Grid Mode: ", gridMode);
                            GUILayout.Space(4);
                        }
        ...
        void DrawPostEvents()
        {
            ...
            _foldout_customEvents = EditorGUILayout.Foldout(_foldout_customEvents, " Custom Post Events:", true);

            if (GUILayout.Button("", EditorStyles.label)) { _foldout_customEvents = !_foldout_customEvents; }

            if (GUILayout.Button("+", FGUI_Resources.ButtonStyle, GUILayout.Height(17), GUILayout.Width(28)))
            {
                _foldout_customEvents = true;
                projectPreset.CustomPostEvents.Add(new FieldSetup.CustomPostEventHelper());
            }

            EditorGUILayout.EndHorizontal();

            if (_foldout_customEvents)
            {
                if (projectPreset.CustomPostEvents.Count == 0)
                    EditorGUILayout.LabelField("No Custom Post Events Added Yet", EditorStyles.centeredGreyMiniLabel);
                else
                    GUILayout.Space(4);

                int toRemove = -1;
                for (int i = 0; i < projectPreset.CustomPostEvents.Count; i++)
                {
                    var ev = projectPreset.CustomPostEvents[i];
                    ...
                    ev.PostEvent = (FieldSpawnerPostEvent_Base)EditorGUILayout.ObjectField(ev.PostEvent, typeof(FieldSpawnerPostEvent_Base), false);
                    ...
                    if (ev.Foldout)
                        DisplayPostEvent(projectPreset.CustomPostEvents[i]);

                    EditorGUILayout.EndVertical();
                }


                if (toRemove > -1) projectPreset.CustomPostEvents.RemoveAt(toRemove);
            }
        ...
        void DisplayPostEvent(FieldSetup.CustomPostEventHelper helper)
        {
            if (helper.PostEvent == null) return;

            if (string.IsNullOrWhiteSpace(helper.PostEvent.PostEventInfo) == false)
                EditorGUILayout.HelpBox(helper.PostEvent.PostEventInfo, MessageType.None);

            helper.PostEvent.Editor_DisplayGUI(helper);
        }


        bool _foldout_customEvents = false;
    }
}