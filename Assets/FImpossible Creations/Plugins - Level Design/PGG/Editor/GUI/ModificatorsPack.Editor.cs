        ...
        internal static void DrawUtilityModsList(List<FieldModification> toDraw, ref bool foldout, string title, FieldSetup setup)
        {
            ...
            if (foldout)
            {
                ...
                if (toRemove >= 0)
                {
                    ...
                    if (setup.UtilityModificators[toRemove] == null) can = true;

                    if (can == false)
                    {
                        if (!setup.UtilityModificators.Contains(toDraw[toRemove]))
                        {
                            bool contains = false;

                            for (int m = 0; m < setup.ModificatorPacks.Count; m++)
                                if (setup.ModificatorPacks[m].FieldModificators.Contains(toDraw[toRemove])) { contains = true; break; }

                            if (!contains) can = true;
                        }
                    }

                    if (can)
                    {
                        toDraw.RemoveAt(toRemove);
                        EditorUtility.SetDirty(setup);
                    }
        ...
        private static void MergeOntoDialog(ModificatorsPack addTo)
        {
            ...
            try
            {
                if (path != "")
                {
                    ...
                    if (pack == addTo)
                    {
                        UnityEngine.Debug.Log("[FGenerators] Can't merge with self!");
                    }
                    else
                    {
                        if (pack != null)
                        {
                            int initCount = pack.FieldModificators.Count;
