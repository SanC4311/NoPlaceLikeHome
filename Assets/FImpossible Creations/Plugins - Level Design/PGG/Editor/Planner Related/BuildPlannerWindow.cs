...
namespace FIMSpace.Generating
{
    public partial class BuildPlannerWindow : EditorWindow
    {
        ...
        public void OnGUI()
        {
            ...
            //if (FieldPlannerWindow.Get) if (PlanGenerationPrint.SelectedPlannerResult == null)
            //    {
            ...
            //                        PlanGenerationPrint.SelectedPlannerResult = projectPreset.BasePlanners[id].LatestResult;
            //                }
            //    }
            ...
            if (drawPrevSettings)
            {
                ...
                if (so_preset != null)
                {
                    var sp = so_preset.FindProperty("AsyncGenerating");
                    if (sp != null)
                    {
                        GUILayout.Space(4);
                        EditorGUIUtility.labelWidth = 0;
                ...
                if (StartupRefs.FieldPlannerDraftsDirectory)
                {
                    ...
                    if (GUILayout.Button(new GUIContent("New", "Generates new Planner Setup file in the PGG drafts directory, after that you can move draft file into project directories with 'Move' button which will appear"), GUILayout.Width(44)))
                    {
                        ...
                        if (string.IsNullOrEmpty(path))
                            path = FGenerators.GenerateScriptablePath(scrInstance, "FS_NewBuildPlanner");
            ...
            if (foldout)
            {
                ...
                if (toDraw.Count > 0)
                {
                    for (int i = 0; i < toDraw.Count; i++)
                    {
                        ...
                        GUI.color = plannerColor * 1.5f;

                        if (GUILayout.Button(PGGUtils._PlannerIconGray, EditorStyles.label, GUILayout.Width(22), GUILayout.Height(19)))
                        {
                            //selected = i;
                            if (toDraw[i]) tosel = (toDraw[i]);
                            FieldPlannerEditor.Editor_ResetNameEdit();
